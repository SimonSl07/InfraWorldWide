...
describe("resolveClientKey", () => {
  ...
  it("falls back through the header order", () => {
    expect(resolveClientKey(headers({ "cf-connecting-ip": "203.0.113.8" })).key).toBe(
      "203.0.113.8",
    );
  ...
  it("marks x-forwarded-for untrusted, since a client can set it", () => {
    expect(resolveClientKey(headers({ "x-forwarded-for": "203.0.113.11" })).trusted).toBe(
      false,
    );
  });
  ...
  it("reports no identity rather than collapsing everyone into one bucket", () => {
    ...
    expect(resolveClientKey(headers({}))).toEqual({ key: null, trusted: false });
  });

  it("ignores blank and whitespace-only header values", () => {
    expect(resolveClientKey(headers({ "x-forwarded-for": "   " })).key).toBeNull();
    expect(resolveClientKey(headers({ "x-forwarded-for": " , 203.0.113.12" })).key).toBe(
      "203.0.113.12",
    );
  });
});
