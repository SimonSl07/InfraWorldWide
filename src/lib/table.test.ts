...
describe("sortRows", () => {
  ...
  it("sorts numbers descending", () => {
    const sorted = sortRows(rows, { columnId: "slip", direction: "desc" }, { valueOf });
    expect(ids(sorted)).toEqual(["e", "a", "d", "c", "b"]);
  });

  it("sorts numbers ascending", () => {
    const sorted = sortRows(rows, { columnId: "slip", direction: "asc" }, { valueOf });
    expect(ids(sorted)).toEqual(["c", "a", "d", "e", "b"]);
  });

  it("keeps unmeasured rows last in both directions", () => {
    for (const direction of ["asc", "desc"] as const) {
      const sorted = sortRows(rows, { columnId: "slip", direction }, { valueOf });
